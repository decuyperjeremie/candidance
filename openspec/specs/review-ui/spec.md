...
Provide a web review surface for stored offers: a detail page that shows an offer's key fields and whether an application exists, the ability to generate an adapted CV and cover letter from that page (reusing the application-generation capability), inline editing and saving of the generated application so that saved edits drive all downloads, and navigation from the offer list to each offer's detail page.
